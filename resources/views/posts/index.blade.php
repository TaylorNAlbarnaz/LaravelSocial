        ...
        <div class="flex flex-col">
            ...
            <ul class="flex flex-col space-y-2">
                @foreach ($posts as $post)
                    <li class="bg-white max-w-3xl p-3 rounded-md shadow-lg">
                        <div class="flex flex-col w-full">
                            <div>
                                <p class="text-xl whitespace-nowrap truncate overflow-hidden font-semibold">{{ $post->user->username }}</p>
                            </div>
                            <p class="text-gray-400 text-sm">{{ $post->created_at->toDateString() }} at {{ $post->created_at->toTimeString() }}</p>
                        </div>
